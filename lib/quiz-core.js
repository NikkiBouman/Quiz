...
/* Display-root voor de bouwer-thumbnails. Puzzel-plaatjes staan root-relatief opgeslagen
   ("puzzle/…", zo werken ze in het spel op de root). De beheerpagina's zitten dieper, dus ze
   zetten hun pad-naar-root (setMediaRoot('../../../')) zodat de thumbnails in de bouwer kloppen.
   Alleen voor wéérgave — wat opgeslagen wordt (clueToStage/clueToFact) blijft canoniek. */
let MEDIA_ROOT = '';
export function setMediaRoot(r){ MEDIA_ROOT = r || ''; }
function dispMedia(p){ if(!p) return p; return (/^(https?:|data:|blob:|\/)/.test(p) || !MEDIA_ROOT || p.startsWith(MEDIA_ROOT)) ? p : MEDIA_ROOT+p; }
...
export function validateQuestions(data){
  ...
  for(let i=0;i<data.length;i++){
    ...
    if(textOpts){
      ...
      if(listOpts){
        const idxs = Array.isArray(ans) ? ans : [ans];   // meerdere juiste opties → lijst van indexen
        if(!idxs.length || idxs.some(x=> typeof x!=='number' || x<0 || x>=q.options.length))
          return `Vraag ${n}: 'answer' moet een geldige index zijn.`;
      }
...
export function qTypeLabel(q){
  if(q.source && q.source.kind==='puzzle') return 'puzzel';
...
/* ============================================================
   ICONEN — één gedeelde, consistente outline-set (currentColor, stroke 2),
   zelfde stijl als ICON_TRASH/ICON_GRIP. Gebruik: icon('eye') of icon('plus', 16).
   Zo staan overal dezelfde, nette SVG's i.p.v. losse emoji's.
   ============================================================ */
const ICON_PATHS = {
  home:    '<path d="M3 10.9 12 3.5l9 7.4"/><path d="M5.4 9.4V20a1 1 0 0 0 1 1H10v-5h4v5h3.6a1 1 0 0 0 1-1V9.4"/>',
  book:    '<path d="M4 5.3C6.9 4.6 9.8 5 12 6.4 14.2 5 17.1 4.6 20 5.3v13.4c-2.9-.7-5.8-.3-8 1.1-2.2-1.4-5.1-1.8-8-1.1z"/><path d="M12 6.4v13.4"/>',
  pencil:  '<path d="M4 20l1.2-4.2L16 5a2 2 0 0 1 3 3L8.2 18.8z"/><path d="M14 7l3 3"/>',
  film:    '<rect x="3.5" y="4.5" width="17" height="15" rx="2.5"/><line x1="8" y1="4.5" x2="8" y2="19.5"/><line x1="16" y1="4.5" x2="16" y2="19.5"/><line x1="3.5" y1="9.5" x2="8" y2="9.5"/><line x1="16" y1="9.5" x2="20.5" y2="9.5"/><line x1="3.5" y1="14.5" x2="8" y2="14.5"/><line x1="16" y1="14.5" x2="20.5" y2="14.5"/>',
  help:    '<circle cx="12" cy="12" r="8.5"/><path d="M9.4 9.3a2.7 2.7 0 0 1 5.3 .8c0 1.8-2.7 2.3-2.7 4.1"/><circle cx="12" cy="17.3" r=".7" fill="currentColor" stroke="none"/>',
  puzzle:  '<path d="M9.5 3.5a1.6 1.6 0 0 1 3.2 0c0 .5-.2 1 .1 1.3.3.3.7.2 1.2.2H17a1 1 0 0 1 1 1v2.7c0 .5-.1.9.2 1.2.3.3.8.1 1.3.1a1.6 1.6 0 0 1 0 3.2c-.5 0-1-.2-1.3.1-.3.3-.2.7-.2 1.2V18a1 1 0 0 1-1 1h-2.7c-.5 0-.9.1-1.2-.2-.3-.3-.1-.8-.1-1.3a1.6 1.6 0 0 0-3.2 0c0 .5.2 1-.1 1.3-.3.3-.7.2-1.2.2H6a1 1 0 0 1-1-1v-2.7c0-.5.1-.9-.2-1.2-.3-.3-.8-.1-1.3-.1a1.6 1.6 0 0 1 0-3.2c.5 0 1 .2 1.3-.1.3-.3.2-.7.2-1.2V6a1 1 0 0 1 1-1h2.7c.5 0 .9.1 1.2-.2.3-.3.1-.8.1-1.3z"/>',
  plus:    '<line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>',
  eye:     '<path d="M2.5 12S6 5.5 12 5.5 21.5 12 21.5 12 18 18.5 12 18.5 2.5 12 2.5 12z"/><circle cx="12" cy="12" r="3"/>',
  upload:  '<path d="M12 15.5V4.5"/><path d="M7.5 9 12 4.5 16.5 9"/><path d="M4.5 15v3a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2v-3"/>',
  download:'<path d="M12 4.5v11"/><path d="M7.5 11 12 15.5 16.5 11"/><path d="M4.5 16v2a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2v-2"/>',
  ...
  volume:  '<path d="M11 5 6.5 9H3.5v6h3L11 19z" fill="currentColor" stroke="none"/><path d="M15.4 8.6a5 5 0 0 1 0 6.8"/><path d="M18.3 6a9 9 0 0 1 0 12"/>',
  link:    '<path d="M9.5 14.5 14.5 9.5"/><path d="M11 6.6l1.3-1.3a4 4 0 0 1 5.6 5.6L16.6 12"/><path d="M13 17.4l-1.3 1.3a4 4 0 0 1-5.6-5.6L7.4 12"/>',
  desktop: '<rect x="3.5" y="4.5" width="17" height="11" rx="2"/><line x1="9" y1="20" x2="15" y2="20"/><line x1="12" y1="15.5" x2="12" y2="20"/>',
  phone:   '<rect x="7.5" y="3.5" width="9" height="17" rx="2"/><line x1="11" y1="17.5" x2="13" y2="17.5"/>',
  close:   '<line x1="6.5" y1="6.5" x2="17.5" y2="17.5"/><line x1="17.5" y1="6.5" x2="6.5" y2="17.5"/>',
  check:   '<path d="M5 12.5 10 17.5 19 7"/>',
  list:    '<line x1="9" y1="6" x2="20" y2="6"/><line x1="9" y1="12" x2="20" y2="12"/><line x1="9" y1="18" x2="20" y2="18"/><circle cx="4.6" cy="6" r="1.4" fill="currentColor" stroke="none"/><circle cx="4.6" cy="12" r="1.4" fill="currentColor" stroke="none"/><circle cx="4.6" cy="18" r="1.4" fill="currentColor" stroke="none"/>',
  info:    '<circle cx="12" cy="12" r="9"/><line x1="12" y1="11" x2="12" y2="16.5"/><circle cx="12" cy="7.8" r="1.1" fill="currentColor" stroke="none"/>',
};
/* één SVG-icoon als string. `name` = sleutel uit ICON_PATHS, `size` in px (standaard 18). */
export function icon(name, size){
  const p = ICON_PATHS[name]; if(!p) return '';
  const s = size || 18;
  return `<svg class="ic" viewBox="0 0 24 24" width="${s}" height="${s}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:block;flex:none">${p}</svg>`;
}
...
export function navMenuHTML(active, base){
  base = base || '';
  const items = [
    { key:'home',      href:`${base}index.html`,        label:`${icon('home',18)}<span>Home</span>` },
    { key:'quizzes',   href:`${base}account/quizzes/`,   label:`${icon('book',18)}<span>Mijn quizzen</span>` },
    { key:'questions', href:`${base}account/questions/`, label:`${icon('pencil',18)}<span>Mijn vragen</span>` },
    { key:'lists',     href:`${base}account/lists/`,     label:`${icon('list',18)}<span>Mijn lijsten</span>` },
  ];
...
/* TMDB geeft landnamen in het Engels terug; deze map vertaalt de gangbare ISO-codes naar het
   Nederlands. Let op: dit is het PRODUCTIELAND (waar de film/serie gemaakt is), niet altijd waar
   de figuur bedacht is — bij twijfel sleept de maker zelf een eigen 'Land'-pil. */
const COUNTRY_NL = { US:'Verenigde Staten', GB:'Verenigd Koninkrijk', NL:'Nederland', BE:'België',
  FR:'Frankrijk', DE:'Duitsland', ES:'Spanje', IT:'Italië', JP:'Japan', FI:'Finland', SE:'Zweden',
  NO:'Noorwegen', DK:'Denemarken', CA:'Canada', AU:'Australië', NZ:'Nieuw-Zeeland', IE:'Ierland',
  KR:'Zuid-Korea', CN:'China', RU:'Rusland', IN:'India', BR:'Brazilië', MX:'Mexico', AT:'Oostenrijk',
  CH:'Zwitserland', PL:'Polen', CZ:'Tsjechië', PT:'Portugal' };
/* niet-Latijns schrift (Grieks/Cyrillisch/Hebreeuws/Arabisch/Japans/Koreaans/Chinees). Een titel die
   dit bevat (bv. de Japanse Moomin-serie) vervangen we door de NL/EN-vertaling uit TMDB. */
const NON_LATIN = /[Ͱ-ϿЀ-ӿ֐-׿؀-ۿ぀-ヿ㄰-㆏㐀-䶿一-鿿가-힯]/;
/* volledige film-/serie-details (genre/jaar/rating/speelduur/land + regisseur/componist + cast) in één
   call via de proxy (type=details → /{movie|tv}/{id}?append_to_response=credits). Bron voor de
   info-feiten én de acteur-hints van de media-bouwer. media='tv' voor series — TMDB gebruikt daar
   andere veldnamen (name/first_air_date/episode_run_time/created_by), die we hier gelijktrekken. */
export async function movieDetails(id, media){
  const isTv = media==='tv';
  try{
    const res = await fetch(`${TMDB_PROXY}/search?type=details&id=${encodeURIComponent(id)}${isTv?'&media=tv':''}`, {headers:{accept:'application/json'}});
    ...
    // series hebben geen losse 'Director' — de bedenker(s) staan apart in created_by
    const creators = () => {
      const cb=(d.created_by||[]).filter(c=>c.name);
      return cb.length ? { name: cb.map(c=>c.name).join(', '), img:(cb.find(c=>c.profile_path)||{}).profile_path ? `https://image.tmdb.org/t/p/w500${cb.find(c=>c.profile_path).profile_path}` : '' } : null;
    };
    ...
    const runtimeMin = d.runtime || (Array.isArray(d.episode_run_time) ? d.episode_run_time[0] : 0);
    // productieland(en): alle production_countries, anders origin_country; ISO-code → Nederlands
    let countries = (d.production_countries||[]).map(c=> COUNTRY_NL[c.iso_3166_1] || c.name || c.iso_3166_1).filter(Boolean);
    if(!countries.length) countries = (d.origin_country||[]).map(c=> COUNTRY_NL[c] || c).filter(Boolean);
    // titel: niet-Latijns schrift (bv. Japanse serietitel) → val terug op de NL/EN-vertaling uit TMDB
    let title = d.title || d.name || d.original_title || d.original_name || '';
    if(NON_LATIN.test(title)){
      const tr = (d.translations && d.translations.translations) || [];
      const fromTr = code => { const t=tr.find(x=>x.iso_639_1===code && x.data && (x.data.name||x.data.title)); const v=t?(t.data.name||t.data.title):''; return (v && !NON_LATIN.test(v)) ? v : ''; };
      const anyLatinTr = tr.map(x=> x.data && (x.data.name||x.data.title)).find(v=> v && !NON_LATIN.test(v));   // laatste redmiddel: élke Latijnse vertaling
      title = fromTr('nl') || fromTr('en') || anyLatinTr || [d.original_title, d.original_name].find(v=> v && !NON_LATIN.test(v)) || title;
    }
    return {
      id: d.id,
      media: isTv ? 'tv' : 'movie',
      ...
      director: isTv ? creators() : person(['Director']),
      ...
      countries,                                       // productieland(en) in NL; kan afwijken van waar de figuur bedacht is
      revenue: d.revenue || null,                      // box office in $; null = onbekend (series, of film zonder cijfers)
      franchise: isTv ? null : !!d.belongs_to_collection,   // boolean, zonder de naam (TMDB heeft dit niet voor series)
...
/* Deezer-zoeken via JSONP (geen CORS, geen key). Elke track levert een
   ...
   Deezer's standaardvolgorde is al "bekendste eerst" (relevantie+populariteit),
   dus die houden we aan — sorteren op het 'rank'-veld maakt het juist slechter.
   We tonen tot DEEZER_LIMIT resultaten (zoveel als er zijn). */
const DEEZER_LIMIT=50;
export function deezerSearch(q){
  return new Promise(resolve=>{
    const cb='dz_'+Math.random().toString(36).slice(2,9);
    const s=document.createElement('script');
    const done=items=>{ try{ delete window[cb]; }catch(e){} s.remove(); resolve(items); };
    window[cb]=data=>done((data&&data.data?data.data:[]).slice(0,DEEZER_LIMIT).map(t=>({
      id:t.id, label:`${t.artist?t.artist.name+' – ':''}${t.title}`,
      thumb:t.album&&t.album.cover_small?t.album.cover_small:null,           // klein: voor de zoekresultaten-lijst
      cover:(t.album&&(t.album.cover_big||t.album.cover_medium||t.album.cover_small))||null,  // groot: voor de hoes-clue in info/hint
      preview:t.preview||'',
    })));
    s.onerror=()=>done([]);
    s.src=`https://api.deezer.com/search?q=${encodeURIComponent(q)}&output=jsonp&callback=${cb}&limit=${DEEZER_LIMIT}`;
    document.body.appendChild(s);
  });
}
...
/* platte tekst-feiten uit de TMDB-details (regisseur/componist zijn personen, zie newFilmPick).
   Élk feit krijgt een label, zodat de info-pillen zelf-verklarend zijn ("Genre: …", "Productieland: …").
   Genres en landen worden losse pillen — één per waarde, met hetzelfde label — zodat je los kunt
   kiezen welke je toont. */
export function movieFacts(d){
  const f=[];
  const lab=(key,label,value)=> f.push({key, label, value:String(value)});
  lab('type','Type', d.media==='tv' ? 'TV-serie' : 'Film');
  if(d.franchise!=null) lab('franchise','Reeks', d.franchise ? 'Franchise' : 'Standalone');   // alleen films
  (d.countries||[]).forEach((c,i)=> lab('country:'+i,'Productieland', c));
  if(d.year) lab('year','Jaar', d.year);
  (d.genres||[]).forEach((g,i)=> lab('genre:'+i,'Genre', g));
  if(d.runtime) lab('runtime','Speelduur', d.runtime);
  if(d.rating)  lab('rating','TMDB-score', d.rating);
  if(d.votes)   lab('votes','Aantal beoordelingen', Number(d.votes).toLocaleString('nl-NL'));
  if(d.revenue) lab('revenue','Opbrengst', `$${Math.round(d.revenue/1e6).toLocaleString('nl-NL')} mln`);
  else          lab('revenue','Opbrengst', 'onbekend');                               // series → geen box office
  return f;
}
...
export function factsHTML(facts, isHost){
  ...
  const cells=facts.map(f=>{
    ...
    if(f.audio) return isHost
      ...
      : `<span class="pill">${icon('volume',15)} ${f.label?esc(f.label):'audio'}</span>`;
...
export function newFilmPick(d){
  ...
  if(d.director) clues.push(personClue('director', d.media==='tv'?'Bedenker':'Regisseur', d.director.name, d.director.img));
  ...
  return { kind:'film', tmdbId:d.id, deezerId:null, label:d.title, poster:d.poster||null,
           clues, prompt:'Welke film?', answerSearch:true, explanation:'' };
}
...
export function newSongPick(t, det){
  ...
  if(t.cover||t.thumb) clues.push(imageClue('cover','Hoes', t.cover||t.thumb));            // albumhoes (grote versie): sleep naar Info/Hints
  ...
  return { kind:'song', deezerId:t.id, tmdbId:null, label:t.label, preview:t.preview||'', cover:t.cover||t.thumb||'',
           clues, prompt:'Welk nummer is dit?', answerSearch:true, explanation:'' };   // grote hoes opslaan, zodat een opnieuw geladen vraag 'm óók groot toont
}

/* puzzel-figuur → een bewerkbare pick. De plaatjes-stukjes staan al klaar als hints (op volgorde),
   de figuur (+ accept-varianten) ligt al in Antwoord, en — als er een TMDB-film/serie aan gekoppeld
   is — staat die als sleepbare pool rechts (titel, type/franchise/genre/jaar/opbrengst/beoordelingen,
   bedenker/regisseur, acteurs). Sleep de Titel naar Antwoord om de vraag om te draaien naar "uit welke
   film/serie komt dit personage?". details = movieDetails(...) of null (geen koppeling / nog niet geladen). */
export function newPuzzlePick(rd, key, it, details){
  const folder = `${(rd&&rd.folder)||'puzzle'}/${key}`;
  const clues = [];
  // de figuur zelf: standaard het antwoord (handmatig nagekeken), met de accept-varianten erbij
  clues.push({ id:'character', kind:'character', key:'character', role:'Personage',
               value:it.answer||'', accept:(it.accept||[]).slice(), zone:'answer' });
  // de plaatjes-stukjes, in onthul-volgorde, al in de Hints-zone ('Stukje' = neutrale preview, geen spoiler)
  (it.parts||[]).forEach((p,i)=> clues.push({ id:'part:'+i, kind:'image', key:'part:'+i, role:'Stukje', img:`${folder}/${p}`, zone:'hint' }));
  // gekoppelde film/serie → de hele sleepbare pool. Titel is alleen auto-zoekbaar bij films (TV-zoeken
  // kan de speler-app nog niet), dus bij series wordt de titel als antwoord handmatig nagekeken.
  if(details){
    clues.push(titleClue(details.title, details.media==='tv'?null:details.id, ':tmdb'));
    if(details.poster) clues.push(imageClue('poster', details.media==='tv'?'Promo':'Poster', details.poster));
    movieFacts(details).forEach(f=> clues.push(factClue(f)));
    if(details.director) clues.push(personClue('director', details.media==='tv'?'Bedenker':'Regisseur', details.director.name, details.director.img));
    if(details.composer) clues.push(personClue('composer','Componist', details.composer.name, details.composer.img));
    (details.cast||[]).forEach(a=> clues.push(personClue('a:'+a.id, 'Acteur', a.name, a.img)));
  }
  return { kind:'puzzle', tmdbId:details?details.id:null, tmdbMedia:details?details.media:null, deezerId:null,
           label:it.answer||key, answerImage:it.full?`${folder}/${it.full}`:null, folder,
           accept:(it.accept||[]).slice(), clues, prompt:(rd&&rd.heading)||'Wie of wat is dit?', answerSearch:false, explanation:'' };
}
...
/* een opgeslagen film/lied/puzzel-vraag terug in een bewerkbare pick (lossless via q.source.clues) */
export function pickFromQuestion(q){
  const src=q.source||{};
  if(src.kind==='puzzle'){                                          // puzzel: clues zijn lossless bewaard
    return { kind:'puzzle', tmdbId:src.id||null, tmdbMedia:src.tmdbMedia||null, deezerId:null,
             label:src.label||q.answerLabel||'', answerImage:q.answerImage||src.answerImage||null,
             folder:src.folder||'', accept:(src.accept||[]).slice(),
             clues:(src.clues||[]).map(c=>({...c})),
             prompt:q.question||'Wie of wat is dit?', answerSearch: q.options!=null && q.options!==':text',
  ...
  p.explanation = q.explanation||'';                         // uitleg/feitje bij de reveal
  return p;
}
...
function answerSearchMode(p){
  ...
  if(p.kind==='film' || p.kind==='puzzle'){     // puzzel: sleep je de Titel naar Antwoord, dan zoekt de speler de film (TMDB)
...
function clueToFact(c){     // info-subkop
  if(c.kind==='fact'){ const o={ value:c.value }; if(c.label) o.label=c.label; return o; }   // label weglaten = kale pil (geen undefined → Firebase weigert dat)
  if(c.kind==='custom'){ const m=customMedia(c);                  // tekst-pil, of een directe media-link
    if(m){ const o={}; o[m.type]=m.url; return o; }
    return { value:c.value }; }
  if(c.kind==='title' || c.kind==='character') return { value:c.value };
...
function clueToStage(c){    // hint (één voor één)
  ...
  if(c.kind==='fact')   return { text:(c.label?`${c.label}: ${c.value}`:c.value), previewLabel:c.label||'' };
  ...
  if(c.kind==='character') return { text:c.value, previewLabel:c.role||'Personage' };
  if(c.kind==='image')  return { image:c.img, previewLabel:c.role||'' };   // poster/hoes als hint-afbeelding (groot, zonder label → .factmain i.p.v. mini-thumb)
  const cap = personParts(c).join(' · ');
...
function clueMediaKey(c){
  ...
  if(c.kind==='fact')   return 't:'+(c.label?`${c.label}: ${c.value}`:c.value);
  if(c.kind==='title' || c.kind==='character') return 't:'+(c.value||'');
  return '';
}
...
export function buildMediaQuestion(p){
  ...
  const prompt=(p.prompt||'').trim() || (isFilm?'Welke film?':isPuzzle?'Wie of wat is dit?':'Welk nummer is dit?');
  ...
  if(isFilm || isPuzzle){
    ...
    const ai = isPuzzle ? p.answerImage : p.poster;   // puzzel: het complete plaatje; film: de poster
    if(ai) q.answerImage=ai;
  } else {
  ...
  // puzzel: ligt de figuur (tekst) in Antwoord, neem dan z'n accept-varianten mee als goede antwoorden
  if(isPuzzle && q.options===':text' && cluesIn(p.clues,'answer').some(c=>c.kind==='character') && (p.accept||[]).length){
    const have=new Set([String(q.answer), ...((q.accept)||[]).map(String)]);
    const extra=(p.accept||[]).filter(a=>a && !have.has(String(a)));
    if(extra.length) q.accept=[...((q.accept)||[]), ...extra];
  }
  if(!q.stages && !(q.facts&&q.facts.length) && !q.audio)
    return { error: (isFilm||isPuzzle) ? 'Sleep minstens één stukje naar Info of Hints — anders ziet de speler niets om op te raden.' : 'Sleep het fragment (of wat info) naar Info of Hints.' };
  q.source={ kind:isFilm?'tmdb':isPuzzle?'puzzle':'deezer', id:p.tmdbId??p.deezerId, label:p.label, clues:(p.clues||[]).map(c=>({...c})) };
  if(isPuzzle){ if(p.answerImage) q.source.answerImage=p.answerImage; if(p.tmdbMedia) q.source.tmdbMedia=p.tmdbMedia; if(p.folder) q.source.folder=p.folder; if((p.accept||[]).length) q.source.accept=p.accept.slice(); }
  if(!isFilm && !isPuzzle){ if(p.preview) q.source.preview=p.preview; if(p.cover) q.source.cover=p.cover; }
  if((p.explanation||'').trim()) q.explanation=p.explanation.trim();   // uitleg/feitje bij de reveal
...
function clueChipHTML(c, num, withToggles){
  ...
  if(c.kind==='audio'){                       // het fragment: speler + clip-venster, overal versleepbaar
    body = `<div class="cchip-frag"><div class="cchip-frag-h">${icon('music',16)}<b>Fragment</b> <span class="muted" style="font-size:11px">(alleen host)</span></div>
    ...
    const prev = (t!=='text'&&c.value) ? (t==='image'?`<img src="${esc(c.value)}" alt="" class="cchip-img">` : t==='video'?`<span class="cchip-role">${icon('play',12)} video</span>` : `<span class="cchip-role" title="audio speelt alleen op het hostscherm">${icon('volume',13)} host</span>`) : '';
  ...
  } else if(c.kind==='character'){           // de figuur zelf (standaard het antwoord; sleep naar Info/Hints om de vraag om te draaien)
    body = `<span class="cchip-t"><b>${esc(c.role||'Personage')}</b>: ${esc(c.value||'')}</span>`;
  } else if(c.kind==='fact'){
    const lab = c.label ? `<b>${esc(c.label)}</b>: ` : '';      // kale tag-pil (type/franchise/genre/…) heeft geen label
    body = `<span class="cchip-t">${lab}${esc(c.value)}</span>${isSearchable(c)?'<span class="cchip-role" title="Dit veld is compatibel met de zoeken-API">zoekbaar</span>':''}`;
  } else if(c.kind==='image'){               // poster/hoes/puzzelstukje: thumbnail + rol (zelfde in pool en Info/Hints)
    body = `<img src="${esc(dispMedia(c.img))}" alt="" class="cchip-img"><span class="cchip-t">${esc(c.role||'Afbeelding')}</span>`;
...
/* uitleg/feitje-veld — gedeeld door de media- én custom-bouwer (nooit tegelijk zichtbaar, dus één id).
   Wordt teruggelezen in wireMediaBuilder/wireCustomBuilder via #mb-explain. */
function explainFieldHTML(p){
  return `<label class="lbl">Uitleg / feitje <span class="czone-sub">verschijnt pas bij het antwoord (optioneel)</span></label>
    <textarea id="mb-explain" class="sinput m0" rows="2" placeholder="Extra info of een leuk feitje dat spelers bij het antwoord te zien krijgen…">${esc(p.explanation||'')}</textarea>`;
}
...
export function wireMediaBuilder(scope, p, render){
  ...
  const read=()=>{
    ...
    if($('#mb-explain')) p.explanation=$('#mb-explain').value;
...
/* ============================================================
   ...
   ============================================================ */
export function newCustomPick(){ return { kind:'custom', prompt:'', clues:[], answerMode:'open', correct:[''], distractors:[''], poolId:'', explanation:'' }; }
...
export function customPickFromQuestion(q){
  ...
  if(src.kind==='custom' && Array.isArray(src.clues)){
    ...
    return { kind:'custom', prompt:q.question||'', clues,
             answerMode:src.answerMode||'open', poolId:src.poolId||'',
             correct:(src.correct||[]).slice(), distractors:(src.distractors||[]).slice(),
             explanation:q.explanation||'' };
  }
  const nid=()=>'custom:'+Math.random().toString(36).slice(2,7);
  const p={ kind:'custom', prompt:q.question||'', clues:[], answerMode:'open', correct:[], distractors:[], explanation:q.explanation||'' };
...
export function buildCustomQuestion(p){
  ...
  if(!correct.length) return { error: mode==='list' ? 'Vink in de lijst aan welk(e) antwoord(en) juist is/zijn.' : 'Vul minstens één juist antwoord in.' };
  if(mode==='mc'){
    ...
    q.source={ kind:'custom', clues:(p.clues||[]).map(c=>({...c})), answerMode:'mc', correct, distractors:dist };
  } else if(mode==='list'){
    const items=poolItems(p.poolId);
    if(!items || items.length<2) return { error:'Kies een antwoordlijst met minstens 2 opties (de lijst is leeg, te kort of bestaat niet meer).' };
    const inList=correct.filter(v=>items.includes(v));
    if(!inList.length) return { error:'Geen van de gekozen antwoorden staat (nog) in deze lijst — vink ze opnieuw aan.' };
    // bron eerst: applyPoolToQuestion bakt options/answer/search/answerLabel uit de live pool
    q.source={ kind:'custom', clues:(p.clues||[]).map(c=>({...c})), answerMode:'list', poolId:p.poolId, correct:inList, distractors:[] };
    applyPoolToQuestion(q);
  } else {
    q.options=':text'; q.answer=correct[0]; if(correct.length>1) q.accept=correct.slice(1);
    q.answerLabel = correct.join(' / ');
    q.source={ kind:'custom', clues:(p.clues||[]).map(c=>({...c})), answerMode:'open', correct, distractors:[] };
  }
  ...
  if((p.explanation||'').trim()) q.explanation=p.explanation.trim();   // uitleg/feitje bij de reveal
  return { q, label: (q.source.correct||correct)[0]||prompt };
}
...
/* Antwoord = Uit een gedeelde lijst: lijst-keuze + de juiste waarde(n) aanvinken.
   De link naar het lijstbeheer is relatief vanaf account/<x>/new/ (beide bouwer-pagina's
   staan op die diepte). */
function poolPickerHTML(p){
  const pools = Object.values(loadPools());
  if(!pools.length)
    return `<p class="muted hint">Je hebt nog geen antwoordlijsten. Maak er een bij <a href="../../lists/" target="_blank" rel="noopener">Mijn lijsten</a> en kies 'm daarna hier.</p>`;
  const cur = p.poolId && pools.some(x=>x.id===p.poolId) ? p.poolId : '';
  const sel = `<option value="" ${cur?'':'selected'}>— kies een lijst —</option>` +
    pools.map(pl=>`<option value="${esc(pl.id)}" ${pl.id===cur?'selected':''}>${esc(pl.name)} (${(pl.items||[]).length})</option>`).join('');
  const pool = cur ? pools.find(x=>x.id===cur) : null;
  const items = pool ? (pool.items||[]) : [];
  const chosen = new Set((p.correct||[]).map(String));
  const checks = !pool ? `<p class="muted hint">Kies eerst een lijst.</p>`
    : (items.length ? items.map(v=>`<label class="cb-poolopt"><input type="checkbox" class="cb-pool-item" value="${esc(v)}" ${chosen.has(String(v))?'checked':''}><span>${esc(v)}</span></label>`).join('')
                    : `<p class="muted hint">Deze lijst is nog leeg.</p>`);
  const playerNote = pool ? (items.length>6?' Spelers krijgen een zoekveld.':' Spelers krijgen knoppen.') : '';
  const hint = pool
    ? `Gedeeld — <a href="../../lists/?edit=${esc(encodeURIComponent(cur))}" target="_blank" rel="noopener">“${esc(pool.name)}” aanpassen ↗</a>; elke vraag die 'm gebruikt update mee.${playerNote}`
    : `Kies een lijst, of <a href="../../lists/" target="_blank" rel="noopener">maak een nieuwe ↗</a>.`;
  return `<label class="lbl">Antwoordlijst</label>
    <select id="cb-pool" class="sinput m0">${sel}</select>
    <p class="muted hint">${hint}</p>
    ${pool?`<label class="lbl">Welk antwoord is juist?</label><div class="cb-poolopts">${checks}</div>`:''}`;
}
export function customBuilderHTML(p, opts){
  ...
  const correctVals = (mode==='list') ? [] : (((p.correct||[]).length) ? p.correct : ['']);   // open/mc: altijd ≥1 invoerveld
  const corrRows=correctVals.map((v,i)=>ansRow('cb-correct','cb-correct-del',v,i,'Juist antwoord')).join('');
  const distRows=(p.distractors||[]).map((v,i)=>ansRow('cb-dist','cb-dist-del',v,i,'Foute optie')).join('');
  const meerkeuze = (mode==='mc' || mode==='list');           // een gedeelde lijst is óók meerkeuze
  const mcSource = (mode==='list') ? 'list' : 'self';          // subkeuze binnen meerkeuze
  const corrGroup = `<div class="cb-grp"><label class="lbl">Juiste antwoord(en)</label>${corrRows}
        <button type="button" class="btn ghost f13" id="cb-add-correct">+ juist antwoord</button></div>`;
  const selfContent = `${corrGroup}
      <div class="cb-grp"><label class="lbl">Foute opties</label>${distRows}
        <button type="button" class="btn ghost f13" id="cb-add-dist">+ foute optie</button></div>
      <p class="muted hint m0">Meer dan 6 opties totaal? Dan krijgen spelers een zoekveld in plaats van knoppen.</p>
      <button type="button" class="btn ghost f13 cb-share" id="cb-tolist">${icon('list',15)}Maak hier een gedeelde lijst van</button>`;
  const openContent = `${corrGroup}
      <p class="muted hint m0">Spelers typen; jij keurt goed. Alle ingevulde antwoorden gelden als juist.</p>`;
  let answerInner;
  if(!meerkeuze){
    answerInner = `<div class="cb-answer">${openContent}</div>`;
  } else {
    answerInner = `<div class="cb-modes cb-submodes">
        <label><input type="radio" name="cb-mcsource" value="self" ${mcSource==='self'?'checked':''}> Zelf invullen</label>
        <label><input type="radio" name="cb-mcsource" value="list" ${mcSource==='list'?'checked':''}> Uit een lijst <span class="muted">(gedeeld)</span></label>
      </div>
      <div class="cb-answer">${mcSource==='list' ? poolPickerHTML(p) : selfContent}</div>`;
  }
  const answerBox=`<div class="czone"><div class="czone-h">Antwoord</div>
    <div class="cb-modes">
      <label><input type="radio" name="cb-ans" value="open" ${mode==='open'?'checked':''}> Open <span class="muted">(typen, jij keurt)</span></label>
      <label><input type="radio" name="cb-ans" value="mc" ${meerkeuze?'checked':''}> Meerkeuze</label>
    </div>
    ${answerInner}
  </div>`;
...
export function wireCustomBuilder(scope, p, render){
  ...
  const read=()=>{
    if($('#mb-prompt')) p.prompt=$('#mb-prompt').value;
    if($('#mb-explain')) p.explanation=$('#mb-explain').value;
    const top=(scope.querySelector('input[name="cb-ans"]:checked')||{}).value;
    if(top==='open') p.answerMode='open';
    else if(top==='mc'){
      const src=(scope.querySelector('input[name="cb-mcsource"]:checked')||{}).value || (p.answerMode==='list'?'list':'self');
      p.answerMode = src==='list' ? 'list' : 'mc';
    }
    if((p.answerMode||'open')==='list'){
      const sel=$('#cb-pool'); if(sel) p.poolId=sel.value||'';
      p.correct = Array.from(scope.querySelectorAll('.cb-pool-item:checked')).map(c=>c.value);   // juiste = aangevinkte lijst-items
    } else {
  ...
  scope.querySelectorAll('input[name="cb-ans"], input[name="cb-mcsource"]').forEach(r=>r.onchange=()=>{ read(); render(); });
  scope.querySelectorAll('.mb-custom-mtype').forEach(sel=>sel.onchange=()=>{ read(); render(); });   // type wisselen → placeholder/voorbeeld bijwerken
  scope.querySelectorAll('.mb-custom-val').forEach(inp=>inp.onchange=()=>{ read(); render(); });     // link ingevuld → voorbeeld tonen
  scope.querySelectorAll('.mb-custom-del').forEach(b=>b.onclick=()=>{ read(); p.clues=p.clues.filter(x=>x.id!==b.dataset.id); render(); });
  ...
  const psel=$('#cb-pool'); if(psel)psel.onchange=()=>{ read(); p.poolId=psel.value||''; p.correct=[]; render(); };   // andere lijst → vinkjes resetten
  scope.querySelectorAll('.cb-pool-item').forEach(c=>c.onchange=()=>read());
  const tol=$('#cb-tolist'); if(tol)tol.onclick=()=>{          // zelf-ingevulde opties → een gedeelde lijst maken (en de vraag eraan koppelen)
    read();
    const correct=(p.correct||[]).map(s=>String(s||'').trim()).filter(Boolean);
    const dist=(p.distractors||[]).map(s=>String(s||'').trim()).filter(Boolean);
    if(!correct.length){ alert('Vul eerst minstens één juist antwoord in.'); return; }
    const items=[]; const seen=new Set();
    [...correct, ...dist].forEach(v=>{ if(!seen.has(v)){ seen.add(v); items.push(v); } });
    if(items.length<2){ alert('Voeg eerst minstens twee opties toe (juiste + foute) voordat je er een gedeelde lijst van maakt.'); return; }
    const def=(p.prompt||'').replace(/\?+\s*$/,'').trim().slice(0,40);
    const name=(prompt('Naam voor de gedeelde lijst (terug te vinden bij “Mijn lijsten”):', def)||'').trim();
    if(!name) return;
    const id=newPoolId(); savePool({ id, name, items });
    p.poolId=id; p.answerMode='list'; p.correct=correct;     // vraag koppelt nu aan de nieuwe lijst
    render();
  };
  return syncFromDOM;
}
...
/* ---- Antwoordlijsten (gedeelde, herbruikbare keuzelijsten — per apparaat) ----
   Een pool: { id, name, items:[ "…" ] }. Een vraag verwijst ernaar via
   q.source.poolId (answerMode:'list'); applyPoolToQuestion() bakt 'm uit. */
export const POOLS_KEY = 'quiz:pools:v1';      // { id: {id, name, items:[]} }
export function loadPools(){ try{ return JSON.parse(localStorage.getItem(POOLS_KEY)||'{}'); }catch(e){ return {}; } }
function writePools(all){ try{ localStorage.setItem(POOLS_KEY, JSON.stringify(all)); return true; }catch(e){ return false; } }
export function savePool(pool){
  if(!pool || !pool.id) return false;
  const all=loadPools();
  const items=[]; const seen=new Set();
  (pool.items||[]).forEach(s=>{ const v=String(s||'').trim(); if(v && !seen.has(v)){ seen.add(v); items.push(v); } });   // trim + dedup, volgorde behouden
  all[pool.id]={ id:pool.id, name:(pool.name||'').trim()||'Naamloze lijst', items };
  return writePools(all);
}
export function deletePool(id){ const all=loadPools(); delete all[id]; writePools(all); }
export function newPoolId(){ return 'pool:'+Math.random().toString(36).slice(2,9); }

/* pool-id → de live items op dit apparaat (kopie), of null als de lijst ontbreekt */
export function poolItems(id){ const p=loadPools()[id]; return p && Array.isArray(p.items) ? p.items.slice() : null; }

/* Een lijst-vraag (q.source.answerMode==='list') herberekenen tegen de LIVE pool: options =
   de pool-items, answer = index(es) van de juiste waarde(n). De rest van de vraag (punten/
   inzet/hints) blijft met rust. Zo updaten alle vragen die naar één lijst verwijzen mee zodra
   je die lijst aanpast. Geeft true bij succes; false als de pool weg is of geen van de juiste
   waarden meer bevat — dan blijven de eerder ingebakken options staan (cross-device vangnet). */
export function applyPoolToQuestion(q){
  const src=q&&q.source;
  if(!src || src.answerMode!=='list' || !src.poolId) return false;
  const items=poolItems(src.poolId);
  if(!items || items.length<2) return false;   // <2 opties = onbruikbare meerkeuze → onresolved laten (bevroren opties blijven staan)
  const correct=(src.correct||[]).map(String).filter(v=>items.includes(v));
  if(!correct.length) return false;
  q.options=items;
  q.answer = correct.length===1 ? items.indexOf(correct[0]) : correct.map(v=>items.indexOf(v)).sort((a,b)=>a-b);
  if(items.length>6){ q.search=true; delete q.shuffle; } else { q.shuffle=true; delete q.search; }   // >6 → doorzoekbaar; anders tegels
  q.answerLabel=correct.join(' / ');
  delete q.optionsMode;     // her-afleiden (list ↔ listsearch) in normalizeQuiz
  return true;
}
/* alle lijst-vragen in een quiz-object opnieuw resolven tegen de live pools (editor-load + hosten) */
export function resolvePoolQuestions(quiz){
  if(quiz && Array.isArray(quiz.rounds)) quiz.rounds.forEach(rd=>(rd.questions||[]).forEach(q=>applyPoolToQuestion(q)));
  return quiz;
}

/* een quiz klaarzetten om in het spel te hosten, daarna naar index.html. We resolven de
   lijst-vragen hier nog één keer tegen de live pools en bakken ze in, zodat de quiz die naar
   Firebase/spelers gaat self-contained is (de game kent geen pools). */
export function setActiveQuiz(name, quiz){
  try{ const q=resolvePoolQuestions(JSON.parse(JSON.stringify(quiz))); localStorage.setItem(ACTIVE_KEY, JSON.stringify({name, quiz:q})); }catch(e){}
}
...
/* een unieke quiznaam: bestaat 'name' al op dit apparaat, dan "name (2)", "(3)", … */
export function uniqueQuizName(name){
  const saved = loadSavedQuizzes();
  if(!saved[name]) return name;
  let i=2; while(saved[`${name} (${i})`]) i++;
  return `${name} (${i})`;
}

/* ruwe JSON-tekst (uit een geïmporteerd bestand) → een opslaanbaar quiz-object, of een
   foutmelding. Accepteert het export-formaat ({name, rounds}) én een kale vragenlijst.
   → { quiz, name } bij succes, of { error } met een leesbare reden. */
export function parseImportedQuiz(text, fallbackName){
  let raw;
  try{ raw = JSON.parse(text); }
  catch(e){ return { error:'dit is geen geldige JSON ('+e.message+').' }; }
  const err = validateQuiz(raw);
  if(err) return { error: err };
  let quiz;
  if(Array.isArray(raw)){
    quiz = { name:'', rounds:[{ name:'Ronde 1', intro:'', questions: raw.slice() }] };
  } else {
    quiz = { name: raw.name||'', rounds:(raw.rounds||[]).map(rd=>({
      name: rd.name||'Ronde', intro: rd.intro||'', questions: Array.isArray(rd.questions)?rd.questions.slice():[]
    })) };
    if(!quiz.rounds.length) quiz.rounds=[{ name:'Ronde 1', intro:'', questions: flattenQuiz(raw).questions }];
  }
  const name = (quiz.name||'').trim() || (fallbackName||'').trim() || 'Geïmporteerde quiz';
  quiz.name = name;
  return { quiz, name };
}
