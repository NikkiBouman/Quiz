 ...
 */
import { esc, icon, qLabel, qTypeLabel, answerText,
         tmdbSearch, deezerSearch, movieDetails, movieCast, deezerTrack,
         newFilmPick, newSongPick, newPuzzlePick, loadCatalog,
         mediaBuilderHTML, wireMediaBuilder, buildMediaQuestion,
...
export function makeMediaActions(S, render){
  ...
  async function songSearch(){ const e=document.querySelector('#song-q'); const q=e?e.value.trim():''; S.songQuery=q; if(!q){ S.songResults=[]; render(); return; } S.songBusy=true; S.songResults=[]; render(); try{ S.songResults=await deezerSearch(q); }catch(err){ S.songResults=[]; S.err='Zoeken mislukt: '+err.message; } S.songBusy=false; render(); }
...
/* de huidige film/song/puzzel-pick → vraag-object (validatie gedeeld; de aanroeper plaatst 'm) */
export function readMediaPick(S){
  const p = S.view==='film' ? S.filmPick : S.view==='puzzle' ? S.puzzlePick : S.songPick;
  if(!p) return { error:'Kies eerst een film, nummer of figuur.' };
  const { q, error } = buildMediaQuestion(p);
  if(error) return { error };
  return { p, q, isFilm:p.kind==='film', kind:p.kind, label:(q.source&&q.source.label)||p.label };
}

/* ---------- puzzel (kant-en-klare figuren → dezelfde sleep-bouwer als film) ----------
   Een figuur uit de ':text'-ronde(s) van de catalogus opent vooraf-geladen in de media-bouwer:
   plaatjes-stukjes liggen al in Hints, de figuur in Antwoord, en de gekoppelde TMDB-film/serie
   staat als sleepbare pool rechts. Zie newPuzzlePick in quiz-core.js. */
export function makePuzzleActions(S, render){
  async function loadPuzzleCatalog(){
    if(S.puzzleRounds) return;
    S.puzzleBusy=true; render();
    try{ const cat=await loadCatalog(); S.puzzleRounds=(cat||[]).filter(r=>r.mode===':text' && r.items && Object.keys(r.items).length); }
    catch(err){ S.err='Puzzels laden mislukt: '+err.message; S.puzzleRounds=[]; }
    S.puzzleBusy=false; render();
  }
  async function puzzlePick(folder, key){
    const rd=(S.puzzleRounds||[]).find(r=>r.folder===folder); const it=rd&&rd.items[key]; if(!it) return;
    S.puzzleBusy=true; S.err=''; render();
    let details=null;
    if(it.tmdb!=null){ const ref=String(it.tmdb), isTv=ref.startsWith('tv:'), id=isTv?ref.slice(3):ref;
      try{ details=await movieDetails(id, isTv?'tv':'movie'); }catch(e){ details=null; } }
    S.puzzlePick=newPuzzlePick(rd, key, it, details);
    if(it.tmdb!=null && !details) S.err='Let op: TMDB-info kon niet geladen worden — de plaatjes-hints staan wel klaar.';
    S.puzzleBusy=false; render();
  }
  return { loadPuzzleCatalog, puzzlePick };
}
export function puzzleFormHTML(S, opts){ opts=opts||{};
  if(S.puzzlePick){
    const title=(opts.editing?'Puzzel wijzigen':'Puzzel')+': '+esc(S.puzzlePick.label);
    return `<div class="statusbar"><h1 class="h-sm">${title}</h1></div>`
      + mediaBuilderHTML(S.puzzlePick, { addLabel:opts.addLabel||'Opslaan', resetLabel:'Andere figuur', roundPickerHTML:opts.roundPickerHTML, hideAdd:opts.hideAdd });
  }
  const rounds=S.puzzleRounds||[];
  const groups = rounds.map(rd=>{
    const figs=Object.keys(rd.items).map(k=>`<button class="btn ghost searchbtn" data-puzzle="${esc(rd.folder)}|${esc(k)}">${esc(rd.items[k].answer||k)}${rd.items[k].tmdb!=null?'':' <span class="muted">(geen TMDB-info)</span>'}</button>`).join('');
    return rounds.length>1 ? `<div class="czone-sub" style="margin-top:6px">${esc(rd.name||rd.folder)}</div>${figs}` : figs;
  }).join('');
  return `<div class="statusbar"><h1 class="h-sm">Puzzel — kies een figuur</h1></div>
    <p class="muted hint">Kies een figuur; de plaatjes-hints en (indien gekoppeld) film/serie-info staan dan al klaar om te bewerken.</p>
    ${S.puzzleBusy?'<p class="muted center" style="margin:0">Bezig…</p>':''}
    <div class="stack edscroll" id="ed-scroll" style="gap:4px">${groups||'<p class="muted m0">Geen puzzelfiguren gevonden.</p>'}</div>
    <div class="row"><button class="btn ghost" id="${opts.backId||'b-back'}">Terug</button></div>`;
}
/* events voor de puzzel-view (figuur-keuze óf de bouwer). opts: { onAdd, onBack, backId, afterBuilderWire }. */
export function wirePuzzleForms(el, S, render, actions, opts){
  opts=opts||{};
  let sync=null;
  if(S.puzzlePick){
    sync=wireMediaBuilder(el, S.puzzlePick, render);
    ...
    const rs=el.querySelector('#b-mb-reset'); if(rs)rs.onclick=()=>{ S.puzzlePick=null; S.err=''; render(); };
  } else {
    el.querySelectorAll('[data-puzzle]').forEach(b=>b.onclick=()=>{ const v=b.dataset.puzzle, i=v.indexOf('|'); actions.puzzlePick(v.slice(0,i), v.slice(i+1)); });
  }
...
export function editorTopbarHTML(opts){
  ...
  return `<div class="topbar"><span class="tb-actions">
      <button type="button" class="btn ghost sm nolink" id="${opts.backId||'b-topback'}">${esc(opts.backLabel||'← Terug zonder opslaan')}</button>
      ${save}
    </span></div>`;
}
...
/* ---------- vraag-voorbeeld: SIMULATIE en VOORVERTONING ----------
   Twee manieren om een vraag te bekijken, allebei via een overlay met iframe(s) van het échte spel
   (index.html). De vraag gaat via localStorage (`quiz:preview`) naar de iframes. `root` is het pad
   naar de repo-root vanaf de huidige pagina (bijv. '../../' of '../../../'). Er staat er altijd maar
   één tegelijk open; `_pvClose` ruimt de actieve op. */
let _pvClose=null;
export function closeQuestionPreview(){ if(_pvClose) _pvClose(); }

/* SIMULATIE — speel de vraag echt na: host- + spelerscherm naast elkaar in twee iframes. Het
   host-iframe start een (tijdelijk) spel en het speler-iframe doet automatisch mee, zodat je als
   host én speler ervaart hoe de vraag verloopt (inzetten, hints, antwoorden). */
export function openQuestionSimulate(question, root){
  closeQuestionPreview();
  try{ localStorage.setItem('quiz:preview', JSON.stringify(question)); }catch(e){}
  try{ localStorage.removeItem('quiz:preview-code'); }catch(e){}
  const base = root || '';
  const ov=document.createElement('div');
  ov.className='pv-overlay';
  ov.innerHTML = `<div class="pv-modal">
      <div class="pv-bar"><b>${icon('play',15)} Simulatie van deze vraag</b>
        <span class="pv-hint">Links speel je host (vraag/info tonen, volgende hint, antwoord), rechts speler (antwoorden). Ze werken echt samen.</span>
        <button class="btn ghost sm" id="pv-close">${icon('close',15)} Sluiten</button></div>
      <div class="pv-panes">
        <div class="pv-pane host"><div class="pv-label">${icon('desktop',15)} Host-scherm</div>
          <iframe class="pv-frame" src="${esc(base)}index.html#preview=host" title="Host-voorbeeld"></iframe></div>
        <div class="pv-pane player"><div class="pv-label">${icon('phone',15)} Speler-scherm</div>
          <iframe class="pv-frame" src="${esc(base)}index.html#preview=player" title="Speler-voorbeeld"></iframe></div>
      </div></div>`;
  document.body.appendChild(ov);
  const hostFrame = ov.querySelector('.pv-pane.host iframe');
  const onKey=e=>{ if(e.key==='Escape') closeQuestionPreview(); };
  document.addEventListener('keydown', onKey);
  _pvClose=()=>{
    _pvClose=null;
    try{ hostFrame && hostFrame.contentWindow && hostFrame.contentWindow.postMessage({ t:'qz-preview-close' }, '*'); }catch(e){}
    document.removeEventListener('keydown', onKey);
    setTimeout(()=>{ try{ ov.remove(); }catch(e){} }, 200);   // host even tijd geven het spel op te ruimen
    try{ localStorage.removeItem('quiz:preview-code'); }catch(e){}
  };
  ov.querySelector('#pv-close').onclick=closeQuestionPreview;
  ov.addEventListener('click', e=>{ if(e.target===ov) closeQuestionPreview(); });
}

/* VOORVERTONING — één host-iframe met de volledige vraag, álle hints meteen onthuld (statisch, geen
   spel). Laat in één oogopslag zien hoe de vraag er uiteindelijk uitziet, inclusief alle foto's/
   fragmenten die anders pas hint-voor-hint verschijnen. */
export function openQuestionReveal(question, root){
  closeQuestionPreview();
  try{ localStorage.setItem('quiz:preview', JSON.stringify(question)); }catch(e){}
  const base = root || '';
  const ov=document.createElement('div');
  ov.className='pv-overlay';
  ov.innerHTML = `<div class="pv-modal reveal">
      <div class="pv-bar"><b>${icon('eye',15)} Voorvertoning van deze vraag</b>
        <span class="pv-hint">De volledige vraag met álle hints onthuld — zo ziet een speler de vraag uiteindelijk.</span>
        <button class="btn ghost sm" id="pv-close">${icon('close',15)} Sluiten</button></div>
      <div class="pv-panes">
        <div class="pv-pane reveal"><div class="pv-label">${icon('eye',15)} Volledige vraag</div>
          <iframe class="pv-frame" src="${esc(base)}index.html#preview=reveal" title="Voorvertoning"></iframe></div>
      </div></div>`;
  document.body.appendChild(ov);
  const onKey=e=>{ if(e.key==='Escape') closeQuestionPreview(); };
  document.addEventListener('keydown', onKey);
