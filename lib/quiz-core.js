...
/* zelf-getypt stukje (via de "+"-knop) — sleepbaar naar Info/Hints/Antwoord */
export function newCustomClue(){ return { id:'custom:'+Math.random().toString(36).slice(2,7), kind:'custom', role:'Eigen', value:'', zone:'off' }; }
...
function clueToFact(c){     // info-subkop
  ...
  if(c.kind==='custom') return { value:c.value };
...
function clueToStage(c){    // hint (één voor één)
  ...
  if(c.kind==='custom') return { text:c.value };
  if(c.kind==='title')  return { text:c.value };
  const cap = personParts(c).join(' · ');
...
function clueChipHTML(c, num, withToggles){
  ...
  } else if(c.kind==='custom'){                // zelf-getypt stukje (overal versleepbaar)
    body = `<input class="mb-custom-val cchip-input" data-id="${esc(c.id)}" value="${esc(c.value||'')}" placeholder="Eigen tekst…"><span class="cchip-role">Eigen</span><button type="button" class="mini danger mb-custom-del" data-id="${esc(c.id)}" title="verwijderen">${ICON_TRASH}</button>`;
  } else if(c.kind==='title'){
    body = `<span class="cchip-t"><b>Titel</b>: ${esc(c.value||'')}</span>${isSearchable(c)?'<span class="cchip-role">zoekbaar</span>':''}`;
  } else if(c.kind==='fact'){
    body = `<span class="cchip-t"><b>${esc(c.label)}</b>: ${esc(c.value)}</span>${isSearchable(c)?'<span class="cchip-role">zoekbaar</span>':''}`;
...
export function mediaBuilderHTML(p, opts){
  ...
  let answerCtl;
  if(!ans.length){
    answerCtl = `<p class="muted" style="font-size:12px;margin:0">Sleep hierheen wat het antwoord is (bv. de titel). Wat hier ligt, kan geen hint/info zijn.</p>`;
  } else if(sm){
    ...
    answerCtl = `<p class="muted" style="font-size:12px;margin:0">Spelers typen het antwoord; jij beoordeelt.${ans.length>1?' Alle gesleepte stukjes gelden als goed.':''}</p>`;
  }
  ...
  return `
    <div class="mb-grid">
      ...
      <div class="mb-col mb-right stack" style="gap:8px">
        ...
        <p class="muted" style="font-size:12px;margin:0">Sleep een stukje naar <b>Info</b>, <b>Hints</b> of <b>Antwoord</b> ◀</p>
        <div class="czone-drop mb-pool" data-zone="off">${poolBody}</div>
        <button type="button" class="btn ghost" id="mb-addcustom" style="font-size:13px">+ eigen stukje</button>
      </div>
    </div>
