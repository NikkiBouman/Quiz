...
/* ---------- vraag-preview (host- + spelerscherm naast elkaar) ----------
   Hergebruikt het échte spel (index.html) in twee iframes: één in host-modus, één in speler-modus.
   De vraag gaat via localStorage naar de iframes; het host-iframe start een (tijdelijk) spel en het
   speler-iframe doet automatisch mee, zodat host en speler echt samenwerken. `root` is het pad naar
   de repo-root vanaf de huidige pagina (bijv. '../../' of '../../../'). */
let _pvClose=null;
export function closeQuestionPreview(){ if(_pvClose) _pvClose(); }
export function openQuestionPreview(question, root){
  ...
  ov.innerHTML = `<div class="pv-modal">
      <div class="pv-bar"><b>Voorbeeld van deze vraag</b>
