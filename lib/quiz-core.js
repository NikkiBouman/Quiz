...
export function newSongPick(t, facts){
  const clues=[ fragmentClue(t.preview) ].concat((facts||[]).map(factClue));   // fragment + feiten, alles in de pool
  return { kind:'song', deezerId:t.id, tmdbId:null, label:t.label, preview:t.preview||'', cover:t.thumb||'',
           clues, prompt:'Welk nummer is dit?', answerKey:'title', mode:':deezer', customAnswer:'', accept:'' };
}
...
export function pickFromQuestion(q){
  ...
  if(Array.isArray(src.clues) && src.clues.length){
    ...
    // legacy liedje → fragment-clue uit de audio + clip
    const aud=(Array.isArray(q.stages)?(q.stages.find(s=>s.audio)||{}).audio:q.audio)||src.preview||'';
    const c=songClip(q);
    p.clues = aud ? [{ id:'fragment', kind:'audio', key:'fragment', role:'Fragment', preview:aud, start:c.start, end:c.end, zone:'hint' }] : [];
  }
  p.prompt = q.question || (isFilm?'Welke film?':'Welk nummer is dit?');
  Object.assign(p, answerFromQuestion(q, p));
  if(!isFilm) p.cover=src.cover||'';
  return p;
}
...
export function buildMediaQuestion(p){
  ...
  const info=cluesIn(p.clues,'info',p.answerKey).filter(c=>c.kind!=='audio');   // audio kan geen subkop zijn
  ...
  if(isFilm){
    ...
    if(factCount>0){
      ...
      q.stages=seq.map((c,i)=>Object.assign(c.kind==='audio'?fragStage():{text:`${c.label}: ${c.value}`}, {betMultiplier:m[i]}));
...
function dropZoneHTML(zone, title, sub, p){
  ...
  const rows=items.map((c,i)=>clueChipHTML(c, zone==='hint'?i+1:null, true)).join('');
  ...
  return `
    <div class="mb-grid">
      ...
      <div class="mb-col mb-right stack" style="gap:8px">
        ...
        <p class="muted" style="font-size:12px;margin:0">Sleep een stukje naar <b>Info</b> of <b>Hints</b> ◀${isFilm?'':' — ook het 🎵 fragment'}</p>
